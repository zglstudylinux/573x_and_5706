...
enum {
    ...
    RING_FLAG_SCO       = BIT(1),       //SCO音频建立标志
    RING_FLAG_NUM       = BIT(2),       //报号标志
    RING_FLAG_SYNC      = BIT(3),       //是否同步状态标志
    RING_FLAG_INBAND_RING = BIT(4),     //带内铃声标志
};
...
{
#if BT_HFP_INBAND_RING_EN
    if((bt_ring.loc_flag & RING_FLAG_RES) == 0 && bt_sco_is_connected()) {
        GLOBAL_INT_DISABLE();
        bt_ring.loc_flag |= RING_FLAG_SCO | RING_FLAG_INBAND_RING;
...
static void sfunc_bt_ring_res_init(void)
{
    ...
    //不要立刻报号，避免不接听自动挂断时，hfp状态有时延迟1s，导致多报一声ring
...
#endif
    if((bt_ring.loc_flag & RING_FLAG_SCO) && !bt_sco_is_connected()) {
        bt_audio_bypass();

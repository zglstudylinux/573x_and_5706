...
void ab_mate_tws_mult_dev_en_sync(void)
{
    u8 offset = 0;
    tws_sync_data[offset++] = TWS_INFO_MULT_DEV_EN;
    tws_sync_data[offset++] = ab_mate_app.mult_dev.en;
    tws_sync_data_len = offset;
    tws_data_sync_do();
}

void ab_mate_tws_mult_dev_delete_sync(u8 *addr)
{
    u8 offset = 0;
    tws_sync_data[offset++] = TWS_INFO_MULT_DEV_DELETE;
    memcpy(&tws_sync_data[offset], addr, 6);
    offset += 6;
    tws_sync_data_len = offset;
    tws_data_sync_do();
}
...
#endif
#if AB_MATE_MULT_DEV_EN
void ab_mate_tws_info_mult_dev_en_proc(uint8_t *data_ptr, u16 size)
{
    if(ab_mate_app.mult_dev.en != data_ptr[0]){
        ab_mate_app.mult_dev.en = data_ptr[0];
        ab_mate_app.do_flag |= FLAG_MULT_DEV_SET;
    }
}

void ab_mate_tws_info_mult_dev_delete_proc(uint8_t *data_ptr, u16 size)
{
    if (size == 6) {
        u8 *addr = &data_ptr[0];
        TRACE("mult dev delete:");
        TRACE_R(addr, 6);
        bt_delete_link_info_for_addr(addr);
#if AB_MATE_CTKD_EN
        ble_delete_bonding_for_addr(0, addr);
#endif
    }
}
...
#if AB_MATE_MULT_DEV_EN
        case TWS_INFO_MULT_DEV_EN:
            TRACE("TWS_INFO_MULT_DEV_EN\n");
            ab_mate_tws_info_mult_dev_en_proc(p_data, data_len);
            break;
        case TWS_INFO_MULT_DEV_DELETE:
            TRACE("TWS_INFO_MULT_DEV_DELETE\n");
            ab_mate_tws_info_mult_dev_delete_proc(p_data, data_len);
            break;
#endif

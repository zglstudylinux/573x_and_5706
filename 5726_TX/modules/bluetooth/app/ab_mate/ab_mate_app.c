...
static const u8 key_local2app_table[] = {APP_KEY_NONE,          APP_KEY_PLAY_PAUSE,
                                         APP_KEY_PREV,          APP_KEY_NEXT,
                                         APP_KEY_VOL_UP,        APP_KEY_VOL_DOWN,
                                         APP_KEY_REDIALING,     APP_KEY_SIRI,
                                         APP_KEY_LOW_LATENCY,   APP_KEY_ANC,
...
{
    u8 bt_set_scan_val = 0;

    if(ab_mate_app.mult_dev.en){
        bt_set_link_max(2);
        bt_set_scan_val = 0x02;  //可连接
    }else{
        bt_set_link_max(1);
        bt_set_scan_val = 0;     //不可发现不可连接
    }
    bt_set_scan(bt_set_scan_val);
...
int ab_mate_connect_info_get(u8* buf)
{
    ...
    for(u8 i=0; i<cnt; i++){
        ...
        TRACE_R(bt_addr, 6);
        memset(name, 0, 32);
        bt_get_link_info_name(bt_addr, name, 32);
        TRACE("name:%s\n",name);
    ...
    while(read_offset < payload_len){
        switch(payload[read_offset]){
            ...
            case MULT_DEV_CTR_UNPAIR:
                ...
                ab_mate_mult_dev_addr_decrypt(ab_mate_app.mult_dev.discon_addr);
#if BT_TWS_EN
                ab_mate_tws_mult_dev_delete_sync(ab_mate_app.mult_dev.discon_addr);
#endif
                bt_delete_link_info_for_addr(ab_mate_app.mult_dev.discon_addr);
#if AB_MATE_CTKD_EN
...
bool ab_mate_receive_proc(u8 index, u8 con_type, u8 *data, u16 len)
{
    TRACE("ab_mate<--(%d,%d)[%d]:", index, con_type, len);
    TRACE_R(data, 8);
...
void ab_mate_eq_overall_gain_set(void)
{
    ...
    case 2:     //rock
        ...
        break;
    case 3:     //jazz
        music_eq_set_overall_gain(-1);
        break;
    case 4:     //classic
        music_eq_set_overall_gain(-5);
        break;
    case 5:     //country
        music_eq_set_overall_gain(0);
        break;
...
u8 ab_mate_eq_set_do(void)
{
    ...
    if(ab_mate_app.eq_info.mode < AB_MATE_EQ_RES_CNT){
        ...
        music_eq_set_by_num(ab_mate_app.eq_info.mode);
    ...
    for(u8 i=0; i< ab_mate_app.eq_info.band_cnt; i++){
        WDT_CLR();
        music_eq_set_for_index(i,(int)ab_mate_app.eq_info.gain[i]);
    }
...
void ab_mate_spp_connect_callback(u8 index)
{
    ab_mate_connect_proc(1);
}

void ab_mate_spp_disconnect_callback(u8 index)
{
    if(ab_mate_app.con_sta == AB_MATE_CON_SPP){

...
#define HFP_FEATURE         (HFP_BAT_REPORT*BT_HFP_BAT_REPORT_EN) | \
                            ...
                            (HFP_VOIP_RECOGNITION* BT_HFP_VOIP_RECOGNITION_EN)
...
#if BT_HFP_EN
    #define BT_ONLY_DUT_MODE     0  //使用手机直连时, 默认启用所有功能
#else
    #define BT_ONLY_DUT_MODE     1  //关闭手机直连时, 去掉一些标准命令, 仅保留BT_DUT功能(省空间)
#endif
...
bool cfg_bt_only_dut_mode           = BT_ONLY_DUT_MODE;
...
//{
//    if(bt_get_link_info_addr_for_order(connect_addr, order)) {
//        bt_connect_address();
//    }
//}
//
...
#if BT_SNATCH_EN
//自定义禁止抢连的业务场景，返回true表示有业务，禁止抢连
bool bt_snatch_custom_transaction_check(void)
{
#if AB_MATE_APP_EN
bool ab_mate_ota_is_start(void);
    return ab_mate_ota_is_start();
#elif LE_AB_FOT_EN || BT_AB_FOT_EN
    return is_fot_start();
#else
    return false;
#endif

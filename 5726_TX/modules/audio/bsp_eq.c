...
#endif
#if EQ_DAC_DIV_EN
static u32 eq_rx_param[2][5*20+1] AT(.eqtest.rxbuf);
#endif
...
//            music_drc_set_by_param(band_cnt, (u32 *)&eq_rx_buf[10]);
            ...
            u8 ch = EQ_CHL|EQ_CHR;
            u32 *buf = (u32 *)&eq_rx_buf[14];
#if EQ_DAC_DIV_EN
            if (eq_rx_buf[12] & BIT(1)) {
                ch = eq_rx_buf[12] & BIT(2)? EQ_CHR : EQ_CHL;
                if (ch & EQ_CHL) {
                    memcpy(eq_rx_param[0], &eq_rx_buf[14], band_cnt*20 + 4);
                    buf = (u32 *)eq_rx_param[0];
                } else if (ch & EQ_CHR) {
                    memcpy(eq_rx_param[1], (u8*)&eq_rx_buf[14], band_cnt*20 + 4);
                    buf = (u32 *)eq_rx_param[1];
                }
            }
#endif
            music_eq_set_by_param(ch, band_cnt, buf);
        }
    }
...
//
//    music_eq_set_by_res(RES_BUF_EFFECT_EFFECT_BIN, get_dac_eq_effect_idx(num));
}
...
#endif
static struct eq_div_t eq_div[DAC_MAX_CH];      //存放所有div eq的参数(左右声道)
...
void eq_index_init(void)
{
    ...
    for (int i = 0; i < DAC_MAX_CH; i++) {
        eq_div[i].total_gain = 0x800000;
        music_eq_divband_init(i, DAC_EQ_NUM_TOTAL, (u32 *)eq_div[i].coef);
#if DAC_PT_EN
...
void music_eq_set_overall_gain(int gain)
{
    ...
    eq_div[0].total_gain = eq_total_gain_tbl[gain+10];
#if DAC_MAX_CH == 2
    eq_div[1].total_gain = eq_div[0].total_gain;
#endif
}

...
void iodm_test_loopback_set_mic_ch(u32 *channel, u32 *anl_gain, u32 *dig_gain)
{
    if (iodm.loopback_mic == 1) {
        *channel = CH_MIC0;
        *dig_gain = (*dig_gain & ~(0x3f << 0)) | iodm.dig_gain;
    } else if (iodm.loopback_mic == 2) {
        *channel = CH_MIC1;
        *dig_gain = (*dig_gain & ~(0x3f << 6)) | (iodm.dig_gain << 6);
    } else if (iodm.loopback_mic == 3) {
        *channel = CH_MIC2;
        *dig_gain = (*dig_gain & ~(0x3f << 12)) | (iodm.dig_gain << 12);
    } else if (iodm.loopback_mic == 4) {
        *channel = CH_MIC3;
        *dig_gain = (*dig_gain & ~(0x3f << 18)) | (iodm.dig_gain << 18);
    } else if (iodm.loopback_mic == 5) {
        *channel = CH_MIC4;
        *dig_gain = (*dig_gain & ~(0x3f << 24)) | (iodm.dig_gain << 24);
    }
}

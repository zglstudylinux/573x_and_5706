...
void room_reverb_audio_input(u8 *ptr, u32 samples, u32 params)
{
    s16 *rptr = (s16 *)ptr;
    ...
    if (room_reverb_cfg.callback) {
        room_reverb_cfg.callback((u8 *)rptr, samples, params);
    }
}
...
void room_reverb_audio_init(u8 sample_rate, u16 samples, u8 channel)
{
    //管理模块结构体赋值
...
#if WIRELESS_MIC_ECHO_EN && WIRELESS_MIC_ROOM_REVERB_EN && WIRELESS_MIC_PLATE_REVERB_EN

...
与 `5706_RX` 一样：这些形态共用同一套 `sfr.h` 寄存器映射 + 同一套预编译库，差异只在 `config_ab*.h` 宏。**但**本 SDK 有两点独有特性（见第三节）。
...
- L17-18：`FUNC_DEVICE_EN 1`、`FUNC_ADAPTER_EN 1`（**收发一体**，与 RX 不同）
...
### 3.5 平台标识：`cpu.h`（本 SDK 独有点 1 的配套）
...
### 3.6 运行期角色切换（本 SDK 独特点 2，与 RX 最大不同）

RX 编译期固定为接收端；TX 收发一体，运行期由配置工具的 xcfg 位决定：

```mermaid
flowchart TD
    A["config.h 编译期<br/>FUNC_DEVICE_EN=1 且 FUNC_ADAPTER_EN=1<br/>（config_ab5706a_le_mic.h L17-18）"] --> B["烧录时配置工具写入 xcfg 位<br/>wireless_adapter_en（xcfg.h L71）"]
    B --> C["wireless_mic_role_init()<br/>读 xcfg_cb.wireless_adapter_en 设 cfg_wireless_role<br/>（wireless_proc.c L323-336）"]
    C --> D["wireless_role_is_adapter() == cfg_wireless_role<br/>（api_wireless.h L115）"]
    D -->|adapter_en=0| E["作发射端 device<br/>（选 wireless_mic_emit 档）"]
    D -->|adapter_en=1| F["作接收端 adapter<br/>（选 wireless_adapter 档）"]
```

具体代码：

- `5726_TX/projects/microphone/xcfg.h` L71：
  ```c
  u32 wireless_adapter_en : 1;   //无线mic接收端使能
  ```
- `5726_TX/modules/wireless/wireless_proc.c` L323-336：
  ```c
  void wireless_mic_role_init(void) {
      ...
      if (xcfg_cb.wireless_adapter_en) {
          cfg_wireless_role = true;    // 接收端
      } else {
          cfg_wireless_role = false;   // 发送端
      }
  }
  ```
- `5726_TX/libs/bt8910/liba/api_wireless.h` L115：
  ```c
  #define wireless_role_is_adapter()   cfg_wireless_role
  ```

于是同一份 TX 固件，烧 `wireless_mic_emit` 档 = 发送端、`wireless_adapter` 档 = 接收端，无需重新编译。代码里大量 `if (wireless_role_is_adapter())` 分支（如 `modules/wireless/wireless.c` L166/225/246…）根据角色走不同逻辑。
...
flowchart TD
    ...
    B --> K["xcfg.h L71: wireless_adapter_en"]
    K --> L["wireless_proc.c L323-336: 设 cfg_wireless_role"]
...
2. **收发一体**：`FUNC_DEVICE_EN` 与 `FUNC_ADAPTER_EN` 同时为 1，角色由 xcfg 位 `wireless_adapter_en` 在运行期决定，一份固件两种用法。

...
enum {
    MIC_PACC_EQ_CS           = 0,
    MIC_PACC_DRC_CS,
    MIC_PACC_FREQ_SHIFT_CS,
    MIC_PACC_MAX_CS,

    //以下是enable bit
    MIC_PACC_EQ_BIT     = (1<<MIC_PACC_EQ_CS),
    MIC_PACC_DRC_BIT    = (1<<MIC_PACC_DRC_CS),
    MIC_PACC_FREQ_SHIFT_BIT    = (1<<MIC_PACC_FREQ_SHIFT_CS),
};

typedef struct {
    void *ctl;
    void *cs[MIC_PACC_MAX_CS];
} mic_pacc_t;

extern mic_pacc_t mic_pacc;
void mic_pacc_init(u8 in_fmt, u8 out_fmt);
void mic_pacc_enable(uint en_flag);
#define mic_pacc_proc(dst, src, samples)         pacc_ctl_proc(mic_pacc.ctl, dst, src, samples)
void mic_pacc_exit(void);

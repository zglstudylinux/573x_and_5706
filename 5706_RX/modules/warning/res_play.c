...
bool bt_is_enable(void) {return false;}
...
static void res_play_dec_init(void)
{
    ...
    res_play.dac_spr = (DACDIGCON0 & 0x3c) >> 2;        //记录当前采样率
...
#if KARAOK_VOICE_RM_EN
        res_play.rm_sta = karaok_get_voice_rm_sta();
        karaok_voice_rm_set(0);                         //播放提示音之前，关闭消元音，避免消原音吞掉提示音
#endif
...
static void res_play_dec_restore(void)
{
    ...
    dac_spr_set(res_play.dac_spr);                    //播放完提示音后，还原采样率
    dac_fade_in();
}
...
uint8_t res_play_add(uint8_t res_idx, uint8_t flag)
{
    ...
    TRACE_INFO("res_play: %d(%d),%x --sta=%d, en=%d\n", res_idx, res_lable_cnt, flag, res_play.state, res_play.enable);
#if BT_TWS_EN
    if(bt_is_tws_mode() && bt_is_enable()) {
...
#else
uint8_t res_play_add(uint8_t res_idx, uint8_t flag){return 0;}
void res_play_add_cb_do(uint16_t param) {}
void res_play_done_cb_do(uint16_t param) {}

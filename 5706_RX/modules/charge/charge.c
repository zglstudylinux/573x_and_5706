...
void charge_init(void)
{
    ...
    if (ctl->const_curr > 15) {
        ...
        if (ctl->const_curr > 79) {                                //5ma一个档位，最大电流配到400ma
            ctl->const_curr = 79;
        }
    }
    ...
    ctl->stop_time        = 18000;               //默认30分钟
    ctl->ldo_en           = 1;                          //默认普通充电使用LDO mode，
    ctl->volt_follow_en   = 0;//ab5700不支持电压跟随, CHARGE_VOLT_FOLLOW_EN;
...
void charge_dc_insert(void)
{
    if (charge_ctl.chag_sta == CHAG_STA_W4_DCIN || charge_ctl.chag_sta == CHAG_STA_OFF) { //5v插入，但是还没开始充电
        ...
        if (charge_ctl.chag_sta == CHAG_STA_ON_TRICKLE) {   //当前是涓流充电
            ...
            if (charge_ctl.chag_sta > CHAG_STA_OFF_VBUS_PATH && charge_off_detect(&charge_ctl)) {

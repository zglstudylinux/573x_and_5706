...
static struct tog_bug_tag howling_dnn_tbuf AT(.buf.howling_dnn);      //乒乓buf控制
static s16 howling_dnn_cache_buf[FRAME_LEN*2] AT(.buf.howling_dnn);   //乒乓buf缓存
static u8 howling_dnn_tmp_buf[PROCESS_OUT_SAMPLES*2] AT(.buf.howling_dnn);    //输出buf中转缓存
static void *howling_dnn_proc_ptr = howling_dnn_cache_buf;
...
static howling_dnn_rdft_t howling_dnn_rdft AT(.buf.howling_dnn);
#endif
...
void howling_dnn_mic_audio_input(u8 *ptr, u32 samples, int ch_mode, void *params)
{
    if (!howling_dnn_mic_cfg.mute && wireless_get_status()) {
        while(samples > 0) {
            ...
            if(tog_buf_get(&howling_dnn_tbuf, howling_dnn_tmp_buf, rlen*2)) {
                tog_buf_rd_toggle(&howling_dnn_tbuf);
            }

            if(tog_buf_put(&howling_dnn_tbuf, ptr, rlen*2)) {
                howling_dnn_proc_ptr = tog_bug_get_w_block(&howling_dnn_tbuf);
                tog_buf_wr_toggle(&howling_dnn_tbuf);
                howling_dnn_mic_proc_kick_start();
            }

            memcpy(ptr, howling_dnn_tmp_buf, rlen*2);
            if (howling_dnn_mic_cfg.callback) {
                howling_dnn_mic_cfg.callback((void *)ptr, rlen, ch_mode, params);
            }
...
void howling_dnn_mic_init(u8 sample_rate, u16 samples, u8 channel)
{
    ...
    tog_buf_init(&howling_dnn_tbuf, howling_dnn_cache_buf, FRAME_LEN*2);
...
#if 0
    print_r(howling_dnn_cache_buf,FRAME_LEN*2);
    memcpy(howling_dnn_cache_buf,test_data,FRAME_LEN*2);
    print_r(howling_dnn_cache_buf,FRAME_LEN*2);
//    GPIOADE |=BIT(4);
//    GPIOAFEN &= ~(BIT(4));
//    GPIOADIR &= ~(BIT(4));
...
#if 0
    howling_dnn_process_ext_window(howling_dnn_cache_buf,NULL,0,howling_dnn_rdft.howling_dnn_fft_in);
    fft_hw(&howling_dnn_rdft.fft_cft);
    howling_dnn_fre_process(howling_dnn_rdft.howling_dnn_fft_out, 0, 0);
    ifft_hw(&howling_dnn_rdft.ifft_cft);
    howling_dnn_process_ext_next(howling_dnn_rdft.howling_dnn_ifft_out,NULL,0,howling_dnn_cache_buf);

#endif
//    howling_dnn_process(howling_dnn_cache_buf,NULL,0);
//    GPIOASET = BIT(5);
//
    printf("out: \n");
    print_r(howling_dnn_cache_buf,FRAME_LEN*2);

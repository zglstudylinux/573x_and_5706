...
#define DNN_L2_RDFT_HW              0                           //是否使用硬件FFT处理,软件fft底层处理若是关闭则无法编译通过
...
typedef struct{
    fft_cfg_t fft_cft;
    ifft_cfg_t ifft_cft;
    s32 dnn_L2_fft_in[512];
    s32 dnn_L2_fft_out[512];
    s32 dnn_L2_ifft_out[512];
}dnn_L2_rdft_t;
static dnn_L2_rdft_t dnn_L2_rdft AT(.data.dnn_L2);
#endif
    ...
    if (!dnn_L2_mic_cfg.mute && wireless_get_status()) {
        while(samples > 0) {
            ...
            if (dnn_L2_mic_cfg.callback) {
                dnn_L2_mic_cfg.callback((void *)ptr, rlen, ch_mode, params);
            }
        ...
        if (dnn_L2_mic_cfg.callback) {
            dnn_L2_mic_cfg.callback((void *)ptr, samples, ch_mode, params);
        }
    }
...
AT(.text.dnn_L2_init)WEAK
void dnn_L2_mic_init(u8 sample_rate, u16 samples, u8 channel)
{
#if DNN_L2_RDFT_HW
    dnn_L2_rdft.fft_cft.size            = RDFT_512;
    dnn_L2_rdft.fft_cft.input_type      = 1;
    dnn_L2_rdft.fft_cft.window_en       = 1;
    dnn_L2_rdft.fft_cft.isr_en          = 0;
    dnn_L2_rdft.fft_cft.in_addr         = dnn_L2_rdft.dnn_L2_fft_in;
    dnn_L2_rdft.fft_cft.out_addr        = dnn_L2_rdft.dnn_L2_fft_out;

    dnn_L2_rdft.ifft_cft.size           = RDFT_512;
    dnn_L2_rdft.ifft_cft.output_type    = 1;
    dnn_L2_rdft.ifft_cft.window_en      = 1;
    dnn_L2_rdft.ifft_cft.isr_en         = 0;
    dnn_L2_rdft.ifft_cft.overlap_en     = 1;
    dnn_L2_rdft.ifft_cft.overlap_len    = 0;
    dnn_L2_rdft.ifft_cft.in_addr        = dnn_L2_rdft.dnn_L2_fft_out;
    dnn_L2_rdft.ifft_cft.out_addr       = dnn_L2_rdft.dnn_L2_ifft_out;

    memset(dnn_L2_rdft.dnn_L2_fft_in, 0, sizeof(dnn_L2_rdft.dnn_L2_fft_in));
    memset(dnn_L2_rdft.dnn_L2_fft_out, 0, sizeof(dnn_L2_rdft.dnn_L2_fft_out));
    memset(dnn_L2_rdft.dnn_L2_ifft_out, 0, sizeof(dnn_L2_rdft.dnn_L2_ifft_out));
#endif
...
#if 0
    print_r(dnn_l2_cache_buf,FRAME_LEN*2);
    memcpy(dnn_l2_cache_buf,test_data,640);
    print_r(dnn_l2_cache_buf,FRAME_LEN*2);
//
//    GPIOACLR = BIT(5);
    dnn_l2_process(dnn_l2_cache_buf);
//    GPIOASET = BIT(5);
//
    printf("out: \n");
    print_r(dnn_l2_cache_buf,FRAME_LEN*2);
...
AT(.text.dnn_L2_exit)WEAK
void dnn_L2_mic_exit(void)
{

}

AT(.text.dnn_L2_set.param)WEAK

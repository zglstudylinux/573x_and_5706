...
extern call_cfg_t *call_cfg_ptr;
...
#define SCO_DUMP_USE_TEMP_BUF       1       //dump数据是否使用缓存buf，减少数据异常的风险
...
static s16 sco_dump_buf[DUMP_SCO_MAX_NB][256] AT(.nr_buf.dump.buf);
#endif
...
{
    if ((call_cfg_ptr == NULL) || (position == 0)) {
        return;
    }
//    nr_cb_t *nr = &call_cfg_ptr->nr;
//
...
{
    if (smic_test_mic_en) {     //mic产测流程不过算法
        return ptr;
    }
    if (!call_cfg_ptr) {
        return ptr;
    }
//    if (call_cfg_ptr->mic_eq_en) {
//        mic_pacc_proc(ptr, ptr, samples);
//    } else {    //走产测流程时，可能不过mic eq，此时硬件后置增益不起效，跑一个软件后置增益
//        mic_post_gain_process_s((s16 *)ptr, call_cfg_ptr->post_gain, samples);
//    }
	if (call_cfg_ptr->nr.nr_cfg_en & NR_CFG_AGC_EN) {
...
{
//    if (!bt_sco_pre_proc(ptr, samples, ch_mode)) {
//        return;
//    }
    //AEC对数据要跳过 bt_sco_adc_to_aec_enc 和 bt_sco_enc_mic_test, 避免打印的同时ADC新数据会覆盖旧数据
    if (call_cfg_ptr) {
        if (bt_sco_adc_to_aec_enc(ptr, samples, ch_mode)) {
            if (!bt_sco_enc_mic_test(ptr, samples, ch_mode)) {          // SMIC_DBG_EN || DMIC_DBG_EN
                return;
            }
            sco_pcm_kick_start();
        }
    } else {
        bt_sco_tx_process(ptr, samples, ch_mode);
    }
}

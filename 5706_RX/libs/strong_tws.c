...
AT(.com_text.weak.bb.btisr)
void bt_acl_get_link_info(uint8_t index, int8_t *rssi ,uint8_t *rx_idx, uint8_t *rxpkt_ok, uint8_t *rxpkt_total) {}
...
AT(.com_text.weak.lc.tws)
uint8_t tws_get_snoop_status(uint8_t index) {
    return 0;
}
AT(.com_text.weak.sbc.play)
...
void tws_set_tx_ack2(uint8_t index) {}
void tws_acl_ignore_ack2(int8_t rssi) {}
#endif

WEAK uint32_t tws_time_tickn_get(void) {return 0;}
WEAK bool tws_time_tickn_expire(uint32_t tickn) {return 0;}
WEAK uint8_t bt_tws_get_status(void) {return 0;}
